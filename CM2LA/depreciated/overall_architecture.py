...
plt.plot(classic_loss_hist)
plt.plot(classic_min_loss_hist[1:])
plt.title("Classic Training History, Rel L2")
plt.yscale('log')
plt.savefig('classic_training_history.png')
plt.close()

# save
model_path = Path(config.get("classic_model_path", "classic_model.eqx"))
meta_path = Path(config.get("classic_meta_path", "classic_model_meta.npz"))
eqx.tree_serialise_leaves(model_path, classic_model)
np.savez_compressed(meta_path, t_grid=np.asarray(t_grid), x_grid=np.asarray(x_grid),
                    trunk_arch=np.asarray(trunk_arch), branch_arch=np.asarray(branch_arch), seed=np.asarray(seed))
print("exported classic model to:", model_path)
...
def rel_l2_vs_time_classic(model, u_test, s_test):
    preds = jax.vmap(lambda u: predict_classic(model, u, tx_grid, t_grid, x_grid))(u_test)
    diff = preds - s_test
    return jnp.linalg.norm(diff, axis=(0, 2)) / (jnp.linalg.norm(s_test, axis=(0, 2)) + eps)
...
classic_rel_curve = rel_l2_vs_time_classic(classic_model, u_test, s_test)

u_rand, s_rand = u_test[20], s_test[20]
classic_pred = predict_classic(classic_model, u_rand, tx_grid, t_grid, x_grid)

for fname, data_img, title, label in [
    ('classic_prediction_grid.png', classic_pred, r"Grid eval $T^*(t,x)b_{NN}$", r"$T(t,x)^* b_{\mathrm{NN}}$"),
    ('classic_true_solution_grid.png', s_rand, "Grid eval $s$", r"$s$"),
    ('classic_error_grid.png', classic_pred - s_rand, r"Error $T^*b_{NN} - s$", r"$T(t,x)^* b_{\mathrm{NN}} - s$"),
]:
    plt.figure(figsize=(7, 4))
    plt.imshow(data_img, extent=[x_grid[0], x_grid[-1], t_grid[-1], t_grid[0]],
...
plt.figure()
plt.plot(np.asarray(t_grid), np.asarray(jax.device_get(classic_rel_curve)))
plt.xlabel("t"); plt.ylabel("mean relative L2 error over x")
plt.title("Classic: Test Rel L2 vs time")
plt.grid(True, alpha=0.3); plt.tight_layout()
plt.savefig("classic_rel_l2_vs_time.png"); plt.close()
...
plt.plot(orth_loss_hist)
plt.plot(orth_min_loss_hist[1:])
plt.title("Orth Trunk Training History, Rel L2, 2-Step")
plt.yscale('log'); plt.savefig('orth_training_history.png'); plt.close()
...
plt.plot(orth_branch_loss_hist)
plt.plot(orth_branch_min_loss_hist[1:])
plt.title("Orth Branch Training History, Rel L2, 2-Step")
plt.yscale('log'); plt.savefig('orth_branch_training_history.png'); plt.close()

# save
trunkA_path = Path(config.get("orth_trunkA_model_path", "orth_trunkA.eqx"))
branch_path = Path(config.get("orth_branch_model_path", "orth_branch.eqx"))
meta_path = Path(config.get("orth_meta_path", "orth_meta.npz"))
qr_path = Path(config.get("orth_qr_path", "orth_qr_factors.npz"))
eqx.tree_serialise_leaves(trunkA_path, orth_model)
eqx.tree_serialise_leaves(branch_path, orth_branch)
np.savez_compressed(meta_path, t_grid=np.asarray(t_grid), x_grid=np.asarray(x_grid),
                    trunk_arch=np.asarray(trunk_arch), branch_arch=np.asarray(branch_arch),
                    num_bases=np.asarray(num_bases), seed=np.asarray(seed))
np.savez_compressed(qr_path, Q=np.asarray(orth_Q), R=np.asarray(orth_R))
print("exported orth model to:", trunkA_path, branch_path)
...
def rel_l2_vs_time_orth(Q, branch_model, u_test, s_test):
    preds = jax.vmap(lambda u: predict_orth(Q, branch_model, u))(u_test)
    diff = preds - s_test
    return jnp.linalg.norm(diff, axis=(0, 2)) / (jnp.linalg.norm(s_test, axis=(0, 2)) + eps)
...
orth_rel_curve = rel_l2_vs_time_orth(orth_Q, orth_branch, u_test, s_test)
orth_pred = predict_orth(orth_Q, orth_branch, u_rand)

for fname, data_img, title, label in [
    ('orth_prediction_grid.png', orth_pred, r"Grid eval $Q^*(t,x)b_{NN}$", r"$Q(t,x)^* b_{\mathrm{NN}}$"),
    ('orth_true_solution_grid.png', s_rand, "Grid eval $s$", r"$s$"),
    ('orth_error_grid.png', orth_pred - s_rand, r"Error $Q^*b_{NN} - s$", r"$Q(t,x)^* b_{\mathrm{NN}} - s$"),
]:
    plt.figure(figsize=(7, 4))
    plt.imshow(data_img, extent=[x_grid[0], x_grid[-1], t_grid[-1], t_grid[0]],
               aspect="auto", origin="upper")
...
plt.figure()
plt.plot(np.asarray(t_grid), np.asarray(jax.device_get(orth_rel_curve)))
plt.xlabel("t"); plt.ylabel("mean relative L2 error over x")
plt.title("Orth: Test Rel L2 vs time")
plt.grid(True, alpha=0.3); plt.tight_layout()
plt.savefig("orth_time_error.png"); plt.close()
...
plt.plot(td_loss_hist)
plt.plot(td_min_loss_hist[1:])
plt.title("Orth TD Trunk Training History, Rel L2, 2-Step")
plt.yscale('log'); plt.savefig('orth_td_training_history.png'); plt.close()
...
plt.plot(td_branch_loss_hist)
plt.plot(td_branch_min_loss_hist[1:])
plt.title("Orth TD Branch Training History, Rel L2, 2-Step")
plt.yscale('log'); plt.savefig('orth_td_branch_training_history.png'); plt.close()

# save
trunkA_path = Path(config.get("orth_td_trunkA_model_path", "orth_td_trunkA.eqx"))
branch_path = Path(config.get("orth_td_branch_model_path", "orth_td_branch.eqx"))
meta_path = Path(config.get("orth_td_meta_path", "orth_td_meta.npz"))
qr_path = Path(config.get("orth_td_qr_path", "orth_td_qr_factors.npz"))
eqx.tree_serialise_leaves(trunkA_path, td_model)
eqx.tree_serialise_leaves(branch_path, td_branch)
np.savez_compressed(meta_path, t_grid=np.asarray(t_grid), x_grid=np.asarray(x_grid),
                    trunk_arch=np.asarray(trunk_arch), branch_arch=np.asarray(td_branch_arch),
                    num_bases=np.asarray(num_bases), seed=np.asarray(seed),
                    N_train=np.asarray(output_tr.shape[2]), T_len=np.asarray(len(t_grid)))
np.savez_compressed(qr_path, Q=np.asarray(td_Q_sign), R=np.asarray(td_R_sign))
print("exported orth_td model to:", trunkA_path, branch_path)
...
def rel_l2_vs_time_td(Q_sign, branch_model, u_test, s_test, t_grid):
    preds = jax.vmap(lambda u: predict_td(Q_sign, branch_model, u, t_grid))(u_test)
    diff = preds - s_test
    return jnp.linalg.norm(diff, axis=(0, 2)) / (jnp.linalg.norm(s_test, axis=(0, 2)) + eps)
...
td_rel_curve = rel_l2_vs_time_td(td_Q_jax, td_branch, u_test, s_test, t_grid)
td_pred = predict_td(td_Q_jax, td_branch, u_rand, t_grid)

for fname, data_img, title, label in [
    ('orth_td_prediction_grid.png', td_pred, r"Grid eval $Q^*(t,x)b_{NN}(t)$, Orth TD", r"$Q(t,x)^* b_{\mathrm{NN}}(t)$"),
    ('orth_td_true_solution_grid.png', s_rand, "Grid eval $s$", r"$s$"),
    ('orth_td_error_grid.png', td_pred - s_rand, r"Error $Q^*b_{NN}(t) - s$, Orth TD", r"$Q(t,x)^* b_{\mathrm{NN}}(t) - s$"),
]:
    plt.figure(figsize=(7, 4))
    plt.imshow(data_img, extent=[x_grid[0], x_grid[-1], t_grid[-1], t_grid[0]],
               aspect="auto", origin="upper")
...
plt.figure()
plt.plot(np.asarray(t_grid), np.asarray(jax.device_get(td_rel_curve)))
plt.xlabel("t"); plt.ylabel("mean relative L2 error over x")
plt.title("Orth TD: Test Rel L2 vs time")
plt.grid(True, alpha=0.3); plt.tight_layout()
plt.savefig("orth_td_rel_l2_vs_time.png"); plt.close()
...
print("=" * 60)
print(f"  classic  Test Rel L2: {float(classic_test_rel_l2):.6e}")
print(f"  orth     Test Rel L2: {float(orth_test_rel_l2):.6e}")
print(f"  orth_td  Test Rel L2: {float(td_test_rel_l2):.6e}")
print("=" * 60)

plt.figure()
plt.plot(np.asarray(t_grid), np.asarray(jax.device_get(classic_rel_curve)), label='classic')
plt.plot(np.asarray(t_grid), np.asarray(jax.device_get(orth_rel_curve)), label='orth')
plt.plot(np.asarray(t_grid), np.asarray(jax.device_get(td_rel_curve)), label='orth_td')
plt.xlabel("t"); plt.ylabel("mean relative L2 error over x")
plt.title("Test Rel L2 vs time: all models")
plt.legend(); plt.grid(True, alpha=0.3); plt.tight_layout()
plt.savefig("comparison_rel_l2_vs_time.png"); plt.close()
