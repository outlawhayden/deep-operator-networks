...
def loss_fn(model, inputs, labels):
    ...
    return jnp.mean(jnp.sqrt(jnp.sum(diff.reshape(-1, diff.shape[2]) ** 2, axis=0) + eps)) ## CHANGED LOSS FUNCTION -- SQUARE OF L2 LOSS + EPS TO AVOID VANISHING GRADIENTS
...
U_true = np.asarray(output_tr).reshape(len(t_grid) * len(x_grid), -1)
lstsq_pred = T_MAT_host @ scipy.linalg.lstsq(T_MAT_host, U_true)[0]
lstsq_error = np.mean(np.linalg.norm(U_true - lstsq_pred, axis=0))
print(f"lstsq lower bound: {lstsq_error:.3e}")
