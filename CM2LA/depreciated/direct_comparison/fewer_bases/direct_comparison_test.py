...
bases_td = Q_sign[:, :, num_bases]  # (T, X, 4)
...
bases_classic_tr  = np.array(T_MAT_classic_tr[:, :num_bases].reshape(len(t_grid), len(x_grid), num_bases))
bases_orth_tr     = np.array(Q_orth[:, :num_bases].reshape(len(t_grid), len(x_grid), num_bases))
bases_td_tr       = Q_sign[:, :, :num_bases]
...
fig.suptitle("Basis Functions by Model (Train)", fontsize=16)
...
fig.suptitle("Basis Functions — Two-Step Models (Train)", fontsize=16)
