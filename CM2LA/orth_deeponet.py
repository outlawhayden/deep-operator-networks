...
plt.title(f"Trunk Model Training History, Rel L2, 2-Step")
plt.yscale('log')
...
def branch_loss_fn(branch_model, RA_target, u_train, eps=1e-8):
    ...
    return jnp.mean(jnp.linalg.norm(diff, axis=-1) /
                    (jnp.linalg.norm(RA_target, axis=-1) + eps))
...
plt.title(f"Branch Model Training History, Rel L2, 2-Step")
plt.yscale('log')
...
def test_rel_l2(Q_MAT, branch_model, u_test, s_test, t_grid, x_grid, eps=1e-12):
...
final_test_rel_l2 = test_rel_l2(jnp.asarray(Q_MAT), branch_model, u_test, s_test, t_grid, x_grid)

print("\n==============================")
print(f"Average Test Rel L2 (Q @ b(u)): {float(final_test_rel_l2):.6e}")
print("==============================\n")
...
def rel_l2_error_vs_time_Q_branch(
    Q_MAT,
    branch_model,
    u_test,
    s_test,
    ...
    """
    Returns rel L2 error curve over time, shape (T,).

    rel(n,t) = ||pred(n,t,:) - s(n,t,:)||_2 / (||s(n,t,:)||_2 + eps)
    curve(t) = mean_n rel(n,t)
    """
    preds = jax.vmap(
    ...
    rel = num / (den + eps)          # (N, T)
    rel_curve = jnp.linalg.norm(diff, axis=(0, 2)) / (jnp.linalg.norm(s_test, axis=(0, 2)) + eps)  # (T,)
    return rel_curve

rel_curve = rel_l2_error_vs_time_Q_branch(
